...
async function reverseGeocode(lat, lon) {
  ...
  try {
    const response = await axios.get('https://nominatim.openstreetmap.org/reverse', {
      params: { lat, lon, format: 'json', 'accept-language': 'en', addressdetails: 1 },
      headers: { 'User-Agent': 'KXX-Photo-Bot/1.0' },
      timeout: 2000
    });
...
// overrideMeta: optional EXIF fields pre-extracted by the browser (web upload only).
// Used as a fallback when the server can't read EXIF from the buffer (e.g. already-WebP files).
async function processPhotoBuffer(buffer, fileName, mimeType, caption, overrideMeta = {}) {
  ...
  try {
    ...
    if (isHeic) {
      try {
        processedBuffer = await sharp(buffer).rotate().webp({ quality: 90 }).toBuffer();
      } catch (error) {
        ...
        try {
          ...
          processedBuffer = await sharp(outputBuffer).rotate().webp({ quality: 90 }).toBuffer();
        } catch (convertError) {
    ...
    if (exifData.latitude && exifData.longitude) {
      ...
      gps = { latitude: exifData.GPSLatitude, longitude: exifData.GPSLongitude };
    }
    // Fallback: use GPS from frontend if server couldn't extract it
    if (!gps && !isNaN(overrideMeta.latitude) && !isNaN(overrideMeta.longitude)) {
      gps = { latitude: overrideMeta.latitude, longitude: overrideMeta.longitude };
      console.log('📍 Using GPS from frontend override');
    }
    ...
    // ── Device ───────────────────────────────────────────────────────────────
    const deviceFromExif = exifData.Make && exifData.Model
      ? `${exifData.Make} ${exifData.Model}`.trim()
      : exifData.Model || exifData.Make || null;
    // Fallback to browser-extracted device if EXIF is missing (e.g. already-WebP uploads)
    const device = deviceFromExif || overrideMeta.device || null;
    ...
    await Promise.all([
      r2Client.send(new PutObjectCommand({
        ...
        Body: compressedBuffer, ContentType: 'image/webp',
        CacheControl: 'public, max-age=31536000'
      })),
      r2Client.send(new PutObjectCommand({
        Bucket: R2_BUCKET, Key: thumbFilename,
        Body: thumbnailBuffer, ContentType: 'image/webp',
        CacheControl: 'public, max-age=31536000'
      }))
    ]);
    console.log('✅ Photos uploaded to R2');
    mark('r2-upload');
    ...
    // Fallback: use browser-extracted camera settings if EXIF from buffer is empty
    if (Object.keys(settings).length === 0) {
      if (overrideMeta.aperture)    settings.aperture    = overrideMeta.aperture;
      if (overrideMeta.shutter)     settings.shutter     = overrideMeta.shutter;
      if (overrideMeta.iso)         settings.iso         = overrideMeta.iso.startsWith('ISO') ? overrideMeta.iso : `ISO ${overrideMeta.iso}`;
      if (overrideMeta.focalLength) settings.focalLength = overrideMeta.focalLength;
      if (Object.keys(settings).length > 0) console.log('📷 Using camera settings from frontend override');
    }

    // ── Date ─────────────────────────────────────────────────────────────────
    let dateStr = null;
    const rawDateVal = exifData.DateTimeOriginal || exifData.CreateDate;
    if (rawDateVal) {
    ...
    // Fallback to browser-extracted date
    if (!dateStr && overrideMeta.date) {
      dateStr = overrideMeta.date;
      console.log('📅 Using date from frontend override');
    }
...
module.exports = { initialize, processPhotoBuffer };
